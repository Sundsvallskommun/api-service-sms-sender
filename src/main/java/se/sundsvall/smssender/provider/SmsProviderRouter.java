...
public class SmsProviderRouter {
	...
	PriorityQueue<SmsProvider> initializeProviderQueue(final List<SmsProvider> smsProviders,
		...
		final var filteredSmsProviderQueue = new PriorityQueue<>(smsProviders.stream()
			.filter(filter)
			.toList());
	...
	boolean sendSmsUsingProviderQueue(final Queue<SmsProvider> providerQueue,
		...
		return retryTemplate.execute(context -> {
			...
			try {
				...
				final var message = format("Unable to send %s using %s. Cause: %s", smsType, currentProvider.getName(), e.getMessage());
				LOG.warn(message);
